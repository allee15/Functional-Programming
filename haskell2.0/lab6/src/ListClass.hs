...
length xs = foldr (\_ n -> succ n) zero xs
...
foldl f z xs = foldr (\x g -> (\a -> g (f a x))) id xs z -- hard; use foldr to compute a function which we then apply on z
...
uncons xs = xs (\x xs' -> Just (x, xs')) Nothing
...
reverse xs = foldr (flip cons) nil xs
...
natToList n = foldr (\x xs -> cons (succ x) xs) nil (iterate pred n)
...
factorial (Succ n) = produs (Succ n) (factorial n)
